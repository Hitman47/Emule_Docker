...
def parse_status(raw):
    ...
    for line in raw.split("\n"):
        ...
        if "download" in ll and "kb/s" in ll:
            ...
            if m2:
                info["download_speed"] = float(m2.group(1))
        if "upload" in ll and "kb/s" in ll:
            m2 = re.search(r"([\d.]+)\s*kb/s", ll)
            if m2:
                info["upload_speed"] = float(m2.group(1))
    return info
...
def parse_search_results(raw):
    ...
    for line in raw.split("\n"):
        ...
        if m:
            results.append(
                {"id": int(m.group(1)), "name": m.group(2).strip(), "size": m.group(3), "sources": int(m.group(4))}
            )
        else:
            m2 = re.match(r"^(\d+)\)\s+(.+)", line)
            if m2:
                results.append({"id": int(m2.group(1)), "name": m2.group(2).strip(), "size": "", "sources": 0})
    return results
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def check_auth(self):
        ...
        if f"token={AUTH_TOKEN}" in cookie:
            return True
    ...
    def do_GET(self):
        ...
        if path == "/favicon.ico":
            self.serve_file(ICONS_DIR / "favicon.ico", "image/x-icon")
            return

        if path.startswith("/icons/"):
            icon_path = ICONS_DIR / Path(path).name
            ctype = "image/png"
            if icon_path.suffix.lower() == ".ico":
                ctype = "image/x-icon"
            self.serve_file(icon_path, ctype)
            return
        ...
        if path == "/api/status":
            ...
            if not query:
                self.send_json({"error": "q requis"}, 400)
                return
            if stype not in ("kad", "global", "local"):
                stype = "kad"
            ...
            num = qs.get("id", [""])[0]
            if num:
                self.send_json({"ok": True, "output": run_amulecmd(f"download {num}")})
            else:
            ...
            h = qs.get("hash", [""])[0]
            if h:
                self.send_json({"ok": True, "output": run_amulecmd(f"cancel {h}")})
            else:
                self.send_json({"error": "hash requis"}, 400)
            ...
            try:
                subprocess.run(["/opt/scripts/file-organizer.sh"], capture_output=True, timeout=30, check=False)
